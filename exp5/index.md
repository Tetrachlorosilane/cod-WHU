...
> **💡 概念**：单周期 CPU 的一个周期等于最慢组合路径的延迟；pc 复位值取 `0x1bfffffc` 是为了让复位后第一条 `nextpc` 落在 `0x1c000000`（取指地址）。
>
> **⚠️ 易错**：`reset` 是 Chisel `Module` 的 final 成员，学生模块里不能再定义同名信号——本项目统一用 `reset_r`。
>
> **⚠️ 易错**：立即数扩展要分清符号扩展（`si12`/`si16`/`si20`）与零扩展（`ui5`/`ui12`）；混用不会报错，但结果差得离谱。
>
