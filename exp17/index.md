...
exp_judge: 'TlbSpec：16 写 / 16 读 / 26 查 → ----PASS!!!'
...
> **💡 概念**：TLB 是页表项的高速缓存：输入虚页号（vppn）与 asid，命中后输出物理页号与权限位。
>
> **⚠️ 易错**：DMW（直接映射窗口）命中时**不查 TLB**，直接得到物理地址；这一步漏掉会让大部分访存走错路径。
>
> **⚠️ 易错**：表项无效（`e = 0`）与「未命中」是两种不同情况，输出的 `found` 与例外类型都不一样。
>
