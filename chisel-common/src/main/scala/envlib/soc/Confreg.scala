...
// 对应原 Verilog：各实验 code/soc_verify/*/rtl/CONFREG/confreg.v（exp6 起逐字相同，约 666~690 行）
//
// 覆盖：cr0~cr7、led、led_rg0/1、数码管扫描、switch、按键消抖状态机、step 按键、
//       timer、仿真标志（simu_flag/io_simu/open_trace/num_monitor）、虚拟串口。
//
// 参数：
//   weWidth = 1（exp6，soc_dram）或 4（exp7 起，字节写使能）
//   simulation = 对应原 Verilog 的 #(parameter SIMULATION)，影响 simu_flag 复位值
//
// 与 Verilog 的差异：
//   1. timer 原用独立 timer_clk；实验环境 `SIMU_USE_PLL=0 ⇒ timer_clk = cpu_clk`，
//      故合并为同一隐式时钟域。
...
class Confreg(simulation: Boolean = false, weWidth: Int = 4) extends Module {
  val io = IO(new Bundle {
    val resetn = Input(Bool())
    // read and write from cpu
    val conf_en    = Input(Bool())
    val conf_we    = Input(UInt(weWidth.W))
  ...
  // --------------------------------------------------------------------------
  // 写使能：原 exp6 为 `conf_en & conf_we`（1 位），exp7 起为 `conf_en & (|conf_we)`
  // --------------------------------------------------------------------------
  val conf_write = io.conf_en && (if (weWidth == 1) io.conf_we(0) else io.conf_we.orR)
  ...
  // --------------------------------------------------------------------------
  // timer（原用 timer_clk；实验环境 timer_clk = cpu_clk，合并为同一时钟域）
  // --------------------------------------------------------------------------
