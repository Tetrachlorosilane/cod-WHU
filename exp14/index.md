...
> **💡 概念**：类 SRAM 协议靠 `req` / `addr_ok` / `data_ok` 三根线握手：发起请求后要等 `addr_ok`，再等 `data_ok` 才算完成。
>
> **⚠️ 易错**：不能假设 `addr_ok` 与 `data_ok` 的先后或间隔固定——桥接与 Cache 会插入任意等待周期。
>
> **⚠️ 易错**：写请求的 `data_ok` 表示数据已被接收；在此之前不能撤销请求或改变数据。
>
...
> 源文件：[chisel/src/main/scala/exp14/student/MyCpuTop.scala](chisel/src/main/scala/exp14/student/MyCpuTop.scala)
...
//
// 本实验（实践任务14）的要求：从零实现（在 exp13 基础上增量）：
