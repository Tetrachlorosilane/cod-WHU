...
> **💡 概念**：异常发生时要把「出事指令的 PC」写入 CSR.ERA，`ertn` 再回到该 PC；而 `syscall` 是系统调用，返回时应到下一条指令——两者容易混。
>
> **⚠️ 易错**：异常要在流水线里冲刷掉已经进入后段的指令，否则会执行本不该执行的指令。
>
> **⚠️ 易错**：CSR 读写有自己的时序：写入后立即读可能读到旧值，按原书规定的顺序操作。
>
...
> 源文件：[chisel/src/main/scala/exp12/student/MyCpuTop.scala](chisel/src/main/scala/exp12/student/MyCpuTop.scala)
