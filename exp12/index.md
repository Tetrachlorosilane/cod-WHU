...
| 运行所需运行件 | 本实验的 Chisel 测试会读取 `code/func/obj/inst_ram.mif`、`code/func/obj/data_ram.mif` 与 `code/gettrace/golden_trace.txt`，**这三类运行件未收录在本站仓库**（体积 107MB，见 README §2）；请从主仓库 `taskvscode/exp12/` 获取，否则 `MifLoader`/`TraceLoader` 会直接报"找不到 .mif / golden_trace.txt" |
