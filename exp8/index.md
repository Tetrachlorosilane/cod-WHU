...
> **💡 概念**：阻塞（stall）一次完整的处理是「冻结前段 + 清空后段 + 插入气泡」，三者缺一不可：只冻结不清空，错误指令仍会向后流动。
>
> **⚠️ 易错**：stall 期间 pc 与 IF/ID、ID/EX 流水寄存器必须保持原值，否则指令会被重复执行或直接丢失。
>
> **⚠️ 易错**：load-use 相关必须阻塞（前递救不了），这正是本实验与 exp9 的分界点。
>
...
> 源文件：[chisel/src/main/scala/exp8/student/MyCpuTop.scala](chisel/src/main/scala/exp8/student/MyCpuTop.scala)
...
//
// 本实验（实践任务8）的要求：从零实现（在 exp7 基础上增量）：
