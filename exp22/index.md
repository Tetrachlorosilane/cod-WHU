...
> **⚠️ 易错**：store 命中直接写 tag/data；store 缺失按写分配策略要先取行——两条路径的握手次数完全不同。
>
> **⚠️ 易错**：ICache 与 DCache 的一致性由软件用 `CACOP` 维护，硬件不保证；测试时别指望「写了内存取指就能看到」。
>
> **💡 概念**：替换脏行会在总线上产生额外的写事务，验证时要覆盖到这一情形。
>
