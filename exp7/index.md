...
> **💡 概念**：五级流水线的本质是「把组合逻辑切段、在段间插入寄存器」；每级需要的信号都要随指令一起存进流水寄存器，不能临时从后级回读。
>
> **⚠️ 易错**：本实验用 block RAM：读数据晚一拍，取指级必须据此提前发地址。把地址当拍的数据直接用，会得到上一条指令。
>
> **⚠️ 易错**：本实验**不处理**数据相关，func 程序在相关指令之间已插入 NOP；若自己改测试程序，务必保留这些间隔，否则会误判为设计错误。
>
