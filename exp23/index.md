...
> **💡 概念**：`CACOP` 的 op 字段区分作废、写 tag 等操作，index 决定作用范围（整 Cache / 按 index / 按地址）。
>
> **⚠️ 易错**：作废表项后要保证流水线中已取的指令、已发出的访存不再使用旧数据（需要冲刷/等待）。
>
> **⚠️ 易错**：`CACOP` 走的是同一条访存通路，不要把它当成纯 CSR 操作实现。
>
...
> 源文件：[chisel/src/main/scala/exp23/student/MyCpuTop.scala](chisel/src/main/scala/exp23/student/MyCpuTop.scala)
