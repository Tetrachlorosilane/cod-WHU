...
> **💡 概念**：按 2 路组相联、每路 4 KB、行 16 B 计算：组数 = 4 KB × 2 ÷ 16 B = 512 组，因此 index 9 位、行内偏移 4 位、tag 19 位（32 位地址切分）。
>
> **⚠️ 易错**：写命中与写缺失的处理不同：写分配策略下，写缺失要先发 `rd` 填充整行，再写入。
>
> **⚠️ 易错**：替换时脏行必须先写回；LRU 与伪随机只影响性能，先用伪随机跑通正确性，再换成 LRU。
>
