...
> **💡 概念**：TLB 相关例外有多种（TLBL/TLBS/PIL/PIS/TLBR/TLBPPI/TLBPE），靠 ESTAT.ECODE 与 ESUBCODE 区分。
>
> **⚠️ 易错**：CRMD.DA 与 CRMD.PG 的组合决定地址翻译是否启用；取值不对会走错例外路径。
>
> **⚠️ 易错**：`invtlb` 之后要保证流水线中已取的指令不再使用旧的 TLB 结果——需要冲刷。
>
...
> 源文件：[chisel/src/main/scala/exp19/student/MyCpuTop.scala](chisel/src/main/scala/exp19/student/MyCpuTop.scala)
