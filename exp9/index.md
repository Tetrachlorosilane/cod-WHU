...
> **💡 概念**：前递（bypass）是把后段的计算结果提前送到 EX 的输入端；多条指令同时命中时，要按「离得最近的一条」优先。
>
> **⚠️ 易错**：前递只能解决计算型相关；load 后紧跟使用其结果，仍需一拍阻塞。
>
> **⚠️ 易错**：比较源操作数时要排除 r0（恒零），否则会误触发前递，把 0 覆盖成别的值。
>
...
> 源文件：[chisel/src/main/scala/exp9/student/MyCpuTop.scala](chisel/src/main/scala/exp9/student/MyCpuTop.scala)
...
//
// 本实验（实践任务9）的要求：从零实现（在 exp8 基础上增量）：
