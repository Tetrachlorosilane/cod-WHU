...
> **💡 概念**：LoongArch **没有**分支延迟槽（与 MIPS 风格不同）；分支在 EX 级判定后，要把已经错误取入的指令冲刷掉。
>
> **⚠️ 易错**：`beq`/`bne` 的偏移是 16 位符号扩展后左移 2 位；`jirl` 的目标是 rj 的值加偏移，别写成 pc 相对。
>
> **⚠️ 易错**：`ld.w`/`st.w` 必须 4 字节对齐；原实验不要求处理未对齐异常，但访问未对齐地址的行为是不可预期的。
>
...
> 源文件：[chisel/src/main/scala/exp11/student/MyCpuTop.scala](chisel/src/main/scala/exp11/student/MyCpuTop.scala)
...
//
// 本实验（实践任务11）的要求：从零实现（在 exp10 基础上增量）：
