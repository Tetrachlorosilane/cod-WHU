...
> 源文件：[chisel/src/main/scala/exp13/student/MyCpuTop.scala](chisel/src/main/scala/exp13/student/MyCpuTop.scala)（本站内副本）
...
//
// 本实验（实践任务13）的教学意图 = 从零实现（在 exp12 基础上增量）：
...
//
// 实现提示（详细方案见原书 7.1.2 / ../../chisel4agent/09-CPU开发场景速查.md）：
...
| 运行所需运行件 | 本实验的 Chisel 测试会读取 `code/func/obj/inst_ram.mif`、`code/func/obj/data_ram.mif` 与 `code/gettrace/golden_trace.txt`，**这三类运行件未收录在本站仓库**（体积 107MB，见 README §2）；请从主仓库 `taskvscode/exp13/` 获取，否则 `MifLoader`/`TraceLoader` 会直接报"找不到 .mif / golden_trace.txt" |
| ⚠️ 未实测 | Chisel 代码为静态交付，未编译/仿真；逐行对照见 `chisel/MAPPING.md` |

## 参考

- 原任务说明：[`task.md`](task.md)（硬链接到 `taskvscode/exp13/7.1.2实践任务13-添加其它异常支持.md`）
...
[← 实践任务12](../exp12/index.md) ｜ [实践任务14 →](../exp14/index.md)
