...
> **💡 概念**：AXI 是五通道独立握手（AR/R/AW/W/B），每个通道各自 valid/ready，**不能**假设它们同拍完成。
>
> **⚠️ 易错**：`valid` 一旦拉高，在 `ready` 到来之前既不能撤销、也不能改变数据（协议硬性要求）。
>
> **⚠️ 易错**：突发传输的长度由 `len`、`size`、`burst` 共同决定，三者必须一致，否则从设备会读错地址。
>
...
> 源文件：[chisel/src/main/scala/exp15/student/MyCpuTop.scala](chisel/src/main/scala/exp15/student/MyCpuTop.scala)
...
//
// 本实验（实践任务15）的要求：从零实现（在 exp14 基础上增量）：
