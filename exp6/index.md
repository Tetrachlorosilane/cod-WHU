...
exp_judge: 'MyCpuTbSpec：golden_trace 逐条比对（9776 条）'
...
> **⚠️ 易错**：这是**找错**实验：不要见到可疑写法就改。有些写法只是风格不同，请先用波形与 `golden_trace` 比对，定位真正的功能错误。
>
> **💡 概念**：ALU 的操作数顺序写反、位选范围写错（如 `sr64_result[30:0]` 与 `[31:0]`）这类错误不会报编译错，只会让某几位结果不对——必须靠逐条比对发现。
>
> **🧭 方法**：把原 tb 的「逐条打印」改写成断言后，**第一个不一致的 PC** 就是问题最可能出现的位置；从那里往回看两级流水最有性价比。
>
