...
> **💡 概念**：`mulh`/`mulh.wu` 取的是 64 位中间结果的高 32 位；`div`/`mod` 的符号规则与 C 一致（向零取整），余数符号随被除数。
>
> **⚠️ 易错**：有符号与无符号扩展混用是最常见的错误：`addi.w` 是符号扩展、`andi` 是零扩展，同一份立即数生成逻辑必须按指令区分。
>
> **⚠️ 易错**：除零不产生异常，结果未定义，但**不能**让流水线卡住。
>
